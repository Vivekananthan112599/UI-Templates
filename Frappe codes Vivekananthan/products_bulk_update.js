...
frappe.pages['product-bulk-update'].refresh = function(wrapper) {
    ...
    frappe.call({
        method: 'ecommerce_business_store.ecommerce_business_store.api.get_all_products',
...
function updatemenuitems() {
    ...
    for (var i = 0; i < datas.length; i++) {
        ...
        console.log(item);
        frappe.call({
            method: 'ecommerce_business_store.ecommerce_business_store.api.update_products',
            args: {
                name: itemid,
                title: itemTitle,
                sku: itemSKU,
...
function get_category(id) {
    ...
    frappe.run_serially([
        ...
        () => {
            ...
            frappe.call({
                method: 'ecommerce_business_store.ecommerce_business_store.doctype.product.product.get_product',
                args: {
                    name: id
                },
                async: false,
...
function get_all_category_list() {
    frappe.call({
        method: 'ecommerce_business_store.ecommerce_business_store.doctype.product.product.get_all_category_list',
        args: {},
        async: false,
        callback: function(data) {
            if (data.message) {
...
function category_dialog(data1) {
    ...
    categoryDialog.set_primary_action(__('Add'), function() {
        ...
        //  if((selected_category_list).len=0){
        //      frappe.throw('Please select any one the category.')
        // }
        console.log(data1.name)
        frappe.call({
            method: 'ecommerce_business_store.ecommerce_business_store.doctype.product.product.update_product_categories',
            args: {
    ...
    // dialog.$wrapper.find('#mainCategoryList').slimScroll({
    //     height:400
    // })
    // $('body').on('click',()=>{
    //     $('.modal').removeClass('in');
...
function generate_image_html(data1) {
    ...
    // $(frm.doc.event_images).each(function(i, j) {
...
function image_dialog(data1) {
    ...
    frappe.require("assets/frappe/js/page_fileupload.js", function() {
        setTimeout(function() {
            ...
            // upload_files(parseInt(randomuppy), 'event_images')
            // upload_files(parseInt(randomuppy), 'product_images',image_doctype="Product Image")
    ...
    $(imgDialog.$wrapper).find('.img-close').on('click', function() {
        ...
        frappe.confirm(__("Do you want to delete the image?"), () => {
            frappe.call({
                method: 'ecommerce_business_store.ecommerce_business_store.api.delete_current_img',
                args: {
                    childname: imgid,
                    doctype: "Product"
                },
    ...
    $(imgDialog.$wrapper).find('#saveImages').click(function() {
        ...
        if (length > 0) {
            ...
            $(imgDialog.$wrapper).find('div[data-fieldname="files_list"] ul li').each(function() {
                ...
                frappe.call({
                    method: 'ecommerce_business_store.ecommerce_business_store.doctype.product.product.update_image',
                    args: {
                        count: count,
                ...
                //for image name validation-by siva
                /*if (image_name) {
    ...
    $(imgDialog.$wrapper).find('.img-edit').click(function() {
        ...
        // let check_data = frm.doc.event_images.find(obj => obj.name == imgid);
